        ...
        Dropdown = {
            ...
            Load = function(idx, data)
                ...
                if object.Multi then
                    -- For multi-dropdowns, data.value is a table {[name]=true}.
                    -- Always call SetValue — table ~= table is always true (ref compare),
                    -- so we bypass the check entirely and guard nil explicitly.
                    object:SetValue(data.value or {})
                else
                    -- For single dropdowns, compare values directly.
                    -- Also handles the nil-reset case: if saved value is nil but
                    -- the object currently has a default, we still need to clear it.
