...
local SaveManager = {} do
    ...
    -- When true, every Save/Load prints a human-readable summary to the
    -- Roblox output console so you can inspect what was written/read without
    ...
    local function DebugLog(action, name, objects)
        ...
        local lines = { string.format("[SaveManager] %s  →  %q", action, tostring(name)) }

        -- Group by type for a compact but readable table
        local byType = {}
        for _, obj in ipairs(objects or {}) do
            local t = obj.type or "unknown"
            if not byType[t] then byType[t] = {} end
            local entry = tostring(obj.idx)

            if t == "Dropdown" then
                local sel
                if obj.multi and type(obj.value) == "table" then
                    local keys = {}
                    for k in pairs(obj.value) do keys[#keys + 1] = tostring(k) end
                    sel = "{" .. table.concat(keys, ", ") .. "}"
                else
                    sel = tostring(obj.value)
                end
                local listSize = (type(obj.values) == "table") and #obj.values or 0
                entry = entry .. " = " .. sel
                    .. "  [list: " .. listSize .. " item" .. (listSize == 1 and "" or "s") .. "]"
                if listSize > 0 and listSize <= 10 then
                    local opts = {}
            ...
            elseif t == "ColorPicker" then
                entry = entry .. " = #" .. tostring(obj.value)
                    .. (obj.transparency and ("  alpha=" .. string.format("%.2f", obj.transparency)) or "")
            elseif t == "KeyPicker" then
                entry = entry .. " = " .. tostring(obj.key) .. "  mode=" .. tostring(obj.mode)
            elseif t == "Input" then
                entry = entry .. ' = "' .. tostring(obj.text) .. '"'
            end

            byType[t][#byType[t] + 1] = entry
        end

        for typeName, entries in pairs(byType) do
            lines[#lines + 1] = "  [" .. typeName .. "]"
                ...
                if type(data.values) == "table" and #data.values > 0 then
                    ...
                    if same then
                        for i, v in ipairs(data.values) do
                            if object.Values[i] ~= v then same = false break end
                        end
                    end
                    if not same then
    ...
    function SaveManager:Save(name)
        ...
        DebugLog("SAVE", name, data.objects)
        return true
    end
    ...
    function SaveManager:Load(name)
        ...
        DebugLog("LOAD", name, decoded.objects)
        return true
    end
